...
class BatchDataProcess:
    ...
    def process_new_batch(self, batch_data_file):
        if self.dataset_type == 'ART':
            data = pd.read_csv(batch_data_file, header=None)
        ...
        elif self.dataset_type == 'MAGNI':
            ################### MAGNI dataset ###################
            data = pd.read_csv(batch_data_file)
        ...
        elif self.dataset_type == 'OBSERVE':

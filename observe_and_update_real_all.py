...
def test_online():
    ...
    onlineUpdateMoD = OnlineUpdateMoD(
        ...
        current_cliff=None,
        output_cliff_folder=f"real_world_exp/cliff_smallgrid/all",
        save_fig_folder=f"save_fig_online")
    

    for batch in range(1, 5):
        start_time = time.time()
        thor_magni = InThorMagniDataset(config_file, raw_data_file=f'real_world_exp/train/all/b{batch}.csv')
        observed_traj = thor_magni.get_observed_traj_all_area_all_time()
        onlineUpdateMoD.updateMoD(observed_traj, f"b{batch}")
        end_time = time.time()
        passed_time = end_time - start_time
        with open(f"real_world_exp/runtime/all_time.txt", "a") as f:
...
with open(f"real_world_exp/runtime/all_time.txt", "w") as f:
    f.write("batch,time\n")

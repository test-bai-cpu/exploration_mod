...
def compute_for_atc_hour(model_type, threshold, decay_rate=None):
    ...
    for hour in range(9, 21, 1):
        ...
        if model_type in ["all"]:
            ...
            # MoD_file = f"cliff/atc-20121024-ds-full-2cliff.csv"
            MoD_data = utils.read_MoD_data_velocity(MoD_file)
            # MoD_data = utils.read_cliff_map_data_obs_version(MoD_file)
        ...
        print(f"Average NLL: {average_nll}, not find: {not_find}", "decay rate: ", decay_rate, "std: ", np.std(each_nlls))
    return np.mean(nlls), hour_mean_nlls, hour_not_find, hour_std


def compute_for_atc_hour_multiple_dates(model_type, threshold, decay_rate=None):
    dataset_name = "ATC"
    ...
    for atc_date in ["1028", "1031", "1104", "1107", "1111", "1114", "1118", "1121", "1125"]:
        for hour in range(20, 21, 1):
            print(f"-------------------- In time period {hour} -------------------")
            test_data_file = f'resubmit-corl-split-hour-atc-test-data/atc-{atc_date}-{hour}.csv'
...
for decay_rate in [0.1]:
    for model_type in ["online"]:
        ...
        average_nll, hour_mean_nlls, hour_not_find, std_nlls = compute_for_atc_hour(model_type, threshold, decay_rate)
        # average_nll, hour_mean_nlls, hour_not_find = compute_for_atc_hour_multiple_dates(model_type, threshold, decay_rate)
        # average_nll, hour_mean_nlls, hour_not_find = compute_for_atc_last_hour(model_type, threshold, decay_rate)
        ...
        total_file_name = f"online_mod_res_atc/try8/atc_{model_type}_total_v2_{decay_rate}.txt"
        ...
        #     f.write(f"{decay_rate},{average_nll},{std_nlls},{hour_not_find[0]}\n")
